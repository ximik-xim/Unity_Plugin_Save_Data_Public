...
/// <summary>
/// Обертка над логикой отправки данных на сервер
/// Нужна для упрощения итоговой работы с запросами(что бы работать через event callback без всяких индификаторов)
/// </summary>
public class SD_GetServerDataJSWrapper : MonoBehaviour
{
    ...
    private Dictionary<int, SD_ServerRequestDataWrapperGetServerDataJS> _data = new Dictionary<int, SD_ServerRequestDataWrapperGetServerDataJS>();
    
    [SerializeField] 
    private SD_AbsGetServerDataJS _getServerDataJs;
    ...
    /// addDataJs это дополнительные данные для сервера в виде JS(не обяз., может быть пустышкой) пример, если нужно указать доп. инфу, откуда именно брать данные с сервера(из блока А или из блока Б, или все разом)
    /// </summary>
    /// <param name="addDataJs"></param>
    public GetServerRequestData<SD_DataGetRequestServerJSWrapperAddDataJS> GetServerDataJS(string addDataJs = "")
    {
        int id = 0;
        ...
        var data = new SD_ServerRequestDataWrapperGetServerDataJS(id);
        _data.Add(id, data);

        _getServerDataJs.GetServerDataJS(Callback, id, addDataJs, _keyInstanceClass);
    ...
    /// </summary>
    private void Callback(int id, StatusCallBackServer statusServer, SD_DataGetRequestServerJSWrapperAddDataJS data, string keyInstanceClass)
    {
